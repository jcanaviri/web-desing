...
const clock = () => {
  let today = new Date();
  let h = (today.getHours() % 12) + today.getMinutes() / 59; // 22 % 12 = 10pm and the minutes for the angle
...
// Chip close
const chipClose = document.querySelector(".chip__close");
chipClose.onclick = () => {
  const chip = chipClose.parentElement;
  chip.remove();
};
